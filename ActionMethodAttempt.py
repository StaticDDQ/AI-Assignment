...
class Player:
    ...
    def update(self, action):
        ...
        if(self.timer == 128 + 24):
            self.gameState.updatePiece(6)
            self.gameState.declareBoard(6)
        elif(self.timer == 192+24):
            self.gameState.updatePiece(4)
            self.gameState.declareBoard(4)
        #print(self.icon + "'s perspective at time " + str(self.timer) + str(self.gameState.blackPieces))
...
class Gamestate:
    ...
    def updatePiece(self,size):
        if(size == 6):
            self.whitePieces = [x for x in self.whitePieces if((6>=x[0]>=1) and (6>=x[1]>=1))]
            self.blackPieces = [x for x in self.blackPieces if((6>=x[0]>=1) and (6>=x[1]>=1))]
        elif(size == 4):
            self.whitePieces = [x for x in self.whitePieces if((5>=x[0]>=2) and (5>=x[1]>=2))]
            self.blackPieces = [x for x in self.blackPieces if((5>=x[0]>=2) and (5>=x[1]>=2))]
