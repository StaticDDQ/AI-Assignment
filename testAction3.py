...
class Board:
    ...
    def gridInit(self, size):
        grid = {}
        origin = (int)((8-size)/2) # in case grid has shrunk
        for row in range(origin, size+origin):
            for col in range(origin, size+origin):
                grid[col,row] = BLANK
        for corner in [(origin, origin), (origin, origin+size-1), (origin+size-1, origin), (origin+size-1, origin+size-1)]:
            grid[origin] = CORNER
        return grid
    ...
    def updateGridSize(self, size):
        ...
        for tile in self.grid:
            if tile in [(origin,origin), (origin,origin+size-1), (origin+size-1,origin), (origin+size-1,origin+size-1)]:
